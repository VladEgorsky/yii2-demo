...
{
    const NOIMAGE_URL = '/img/no-image.png';
    ...
    /**
     * News[tags] is Kartik Select2 widget that can add new Tags directly into the input
     *
     * @return bool
     * @throws \yii\db\Exception
     */
    public function createNewTags()
    {
        ...
        // For the new tags we have Tag names instead if Ids here
        // For example $this->tagList = ['3', '15', 'newTag1', 'newTag2' ...]
        // 3 & 15 are existing Ids. newTag1 & newTag2 - new names
        $newTagNames = array_diff($this->tagList, Tag::find()->select('id')->asArray()->column());
        if (empty($newTagNames)) {
            return true;
        }

        foreach ($newTagNames as $title) {
            $k = array_search($title, $this->tagList);
            unset($this->tagList[$k]);

            Yii::$app->db->createCommand()->insert('tag', ['title' => $title])->execute();
            $this->tagList[] = Yii::$app->db->lastInsertID;
        }
